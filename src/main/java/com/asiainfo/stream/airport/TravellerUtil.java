...
public class TravellerUtil {
    ...
    String genSigContent(String imsi, long startDate, long endDate, long generateRate) {
        ...
        long days = hours / 24;
        int travelDays = getNotZeroRandomInt(5); // 出现在机场的天数，限制少于5天
        ...
        GenTravellerApp.summaryInfo.append(imsi + ": traveller" + "\t");
        ...
        for (int d = 0; d < days; d++) { // 按天循环
            ...
            if (!(dayth < 0)) { // 当天出现在机场
                int hourInAirport = random.nextInt(24); // 几时信令在机场区域
...
//                System.out.println("new traveller file: " + userFile.getAbsolutePath() + ": " + created);
    ...
    public static void main(String[] args) {
        ...
        try {
            new TravellerUtil().generateTravellerData("100001000002829", TimeUtil.getTime(startDateStr), TimeUtil.getTime(endDateStr), 2L);
